...
class WorkingTimeTests(TestCase):
    ...
    def test_readable_time(self):
        ...
        expected = f"{working_time.from_time.strftime(WorkingTime.TIME_FORMAT)} - midnight"
        ...
        expected = (
            f"{working_time.from_time.strftime(WorkingTime.TIME_FORMAT)} - "
            f"{working_time.to_time.strftime(working_time.TIME_FORMAT)}"
        )
        self.assertEqual(expected, working_time.readable_time)
        ...
        expected = (
            f"{working_time.from_time.strftime(WorkingTime.TIME_FORMAT)} - "
            f"{working_time.to_time.strftime(WorkingTime.TIME_FORMAT)}"
        )
        self.assertEqual(expected, working_time.readable_time)
